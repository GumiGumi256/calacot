...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!design) notFound();
  const title = `${design.title} | Calacot`;
  const description = design.description?.slice(0, 160) || `Explore ${design.title}, specifications and design packages from Calacot.`;
  return {
    title: { absolute: title }, description,
    alternates: { canonical: absoluteUrl(`/architecture/designs/${encodeURIComponent(design.slug)}`) },
    openGraph: { title, description, images: design.featuredImage ? [{ url: urlFor(design.featuredImage).width(1200).height(630).fit("crop").url(), alt: design.title }] : [] },
  };
}
...
export default async function DesignDetailsPage({ params }: Props) {
  ...
  const images = [
    ...
    ...(design.images || []).map((item, index) => ({ src: urlFor(item.image).width(1800).auto("format").url(), alt: `${design.title} — view ${index + 2}`, blur: item.blur })),
  ];
  ...
  return <article className="bg-brand-white text-brand-black dark:bg-brand-black dark:text-brand-white">
    <div className="mx-auto max-w-7xl px-6 pb-24 pt-28 md:px-10 md:pt-28">
      {/* <Link href="/architecture/designs" className="inline-flex min-h-11 items-center gap-2 text-sm opacity-65 hover:opacity-100"><ArrowLeft size={16} aria-hidden="true" />All designs</Link> */}
      ...
      <div className="mt-14 grid gap-12 lg:grid-cols-[minmax(0,1fr)_400px] lg:gap-16">
        <div className="min-w-0 space-y-10">
          <section><h2 className="text-3xl tracking-[-.04em]">About this design</h2>{design.description && <p className="mt-5 whitespace-pre-line text-sm leading-8 opacity-70">{design.description}</p>}</section>
          <section><h2 className="text-2xl tracking-[-.03em]">Design specifications</h2>
          ...
          {!!design.features?.length && <section><h2 className="text-2xl tracking-[-.03em]">Thoughtful details</h2><ul className="mt-5 grid gap-4 sm:grid-cols-2">{design.features.map((feature, index) => <li key={index} className="flex gap-3 text-sm leading-6"><Check className="mt-1 size-4 shrink-0" aria-hidden="true" />{feature}</li>)}</ul></section>}
          {!!design.additionalServices?.length && <section><h2 className="text-2xl tracking-[-.03em]">Additional services</h2><p className="mt-3 text-sm opacity-60">Optional services, priced separately from your design package.</p><dl className="mt-4">{design.additionalServices.map((service) => <div key={service._key} className="flex flex-wrap justify-between gap-3 border-b border-current/15 py-4 text-sm"><dt>{serviceLabels[service.service] || label(service.service)}</dt><dd className="opacity-65">{service.priceOnRequest || service.price == null ? "Price on request" : money(service.price)}</dd></div>)}</dl></section>}
          ...
          {design.canCustomize != null && <section><h2 className="text-2xl tracking-[-.03em]">{design.canCustomize ? "Adapt it to your life" : "Customization"}</h2><p className="mt-4 whitespace-pre-line text-sm leading-8 opacity-70">{design.canCustomize ? design.customizationNote || "This design can be customized. Tell our team about the changes you have in mind." : "This design is offered as shown, without customization."}</p></section>}
        </div>
